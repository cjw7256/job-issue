...
public class PageMaker {
	...
	private SearchItem si; // page(현재 페이지), perPageNum(페이지 당 보여질 게시물의 수)

	public PageMaker(SearchItem si) {
		this.si = si;
	}
	...
	public void calcData() {
		int page = this.si.getPage();
		int perPageNum = this.si.getPerPageNum();
	...
	public String makeQuery(int page, boolean needSearch) {
		UriComponentsBuilder uriComponentsBuilder = UriComponentsBuilder
				...
				.queryParam("perPageNum", this.si.getPerPageNum());
		// 검색 한 경우
		if (this.si.getSearchType() != null) {
			uriComponentsBuilder.queryParam("searchType", this.si.getSearchType()).queryParam("keyword",
					this.si.getKeyword());
		}
